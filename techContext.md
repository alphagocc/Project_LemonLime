...
- 语言与框架：C++17，Qt Widgets；默认启用 Qt6（`-DLEMON_QT6=ON`），可切换 Qt5（5.15+）。少用异常，偏好值语义错误处理。
...
- 平台注意：Windows 指定字体/Hinting；Linux/macOS watcher/limits 在 `unix/watcher_*.cpp/mm`；高 DPI 在 Qt5 需显式 Attribute。日志与缓存位于用户数据目录。
