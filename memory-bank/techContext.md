...
### 1. C++ 编码标准
- **核心原则**: 清晰性优先，遵循 C++ Core Guidelines，严格使用 RAII。
- **错误处理**: 
  - **禁用异常**: 采用基于值的错误处理。
  - **返回值**: 使用 `std::optional<T>` 表示可能无值，`std::variant` 或 `Result` 处理错误。
  - **禁止**: 不返回 `nullptr` 或魔术数字表示失败。
- **宏使用**: 极少使用。优先用 `const`/`constexpr` 和 `inline`/templates。宏命名必须 `ALL_CAPS`。
- **命名约定**:
  - 类型 (类/结构体/枚举): `PascalCase`
  - 函数/变量: `camelCase`
  - 私有成员: `m_` 前缀
  - 常量/枚举值: `k` 前缀 + `PascalCase`
- **现代化特性**:
  - 必须使用智能指针 (`std::unique_ptr`, `std::shared_ptr`)，避免裸 `new`/`delete`。
  - 显式使用 `override` 和 `final`。

### 2. 项目工作流约定
- **构建目录**: 必须使用 `../build` (工作区外部)，严禁在工作区内创建 `build` 文件夹。
- **提交规范**:
  - 代码提交与记忆库(memory-bank)更新提交必须分离。
  - Commit message 必须简洁精炼，不超过 1 行。
- **格式化**: 提交前确保通过 `.clang-format` 格式化。
